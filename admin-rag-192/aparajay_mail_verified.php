...
<?php } ?>
        ...
        $sql="SELECT * FROM `aparajay_student_data`";                       
        // List all records
        ...
        <tr>
            ...
            <td><?php echo $i++?></td>
			<td>SA-<?php echo $row['boardexam']."-".substr("0000000{$row['id']}", -7)?></td>
           ...
           <?php if($row['marksheet_file']!=""){ ?>
            <a class='btn btn-success text-dark text-center mx-1' style="margin:2px;" href='../aparajay_uploads/<?php echo $row["id"]; ?>/marksheet_img/<?php echo $row['marksheet_file']; ?>'>Marksheet</a>
           <?php } ?>
           <a class='btn btn-success text-dark text-center' href='generate_aparajay_card.php?id=<?php echo $row["reg_id"]; ?>'>Pdf Gen</a>
           </td> 
	...
	<input type="submit" class="btn btn-success" name="bulk_aparajay_email_submit" value="Send Email"/>
</form>
